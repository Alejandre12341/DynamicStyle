...
export function resolvePrimitiveValue(value, ctx) {
  // Si es number o boolean, devolver tal cual
  ...
  if (typeof value === "string") {
    ...
    // Si no es ninguno de los anteriores, devolver string tal cual
    return null;
  }

  // Para cualquier otro tipo raro, devolver tal cual
  return null;
}
...
export function parseValue(value, ctx, resolvers = defaultResolvers) {
  ...
  const see=resolvePrimitiveValue(value,ctx)
  
  return see === null?resolveDynamic(value, ctx, resolvers):see;
}
